...
{
	public class ShowReportArticles : GameButtonHandler
	{
		...
		{
			private readonly ShowReportArticles parent;
			private readonly Texture2D tex;
						
			public RenderEntry (ShowReportArticles parent) : base (-1, -1, 512, parent.iconTex)
			{
				this.parent = parent;
				tex = ShowReportArticles.articleTex;
			...
			public override void Render ()
			{
				...
				if (SimpleGUI.Button (new Rect (xOffset + tex.width - 32, yOffset, 32, 32), parent.forwardTex, parent.forwardHTex, black, white)) {
					if (parent.index < GameControl.self.scene.progression.reports.Count - 1) {
						parent.index += 1;
						parent.RenderArticle ();
					}
				}
		...
		public void RenderArticle () {
			string text = GameControl.self.scene.progression.reports[index].text;
			RenderFontToTexture.self.RenderNewsArticle (text, GameControl.self.scene, articleTex, true);		
		}
		...
		public override void UpdateState (GameButton button) {
			button.isVisible = (GameControl.self.scene.progression.gameEnded);
